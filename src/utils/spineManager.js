...
import { spine as PixiSpine, Texture as PixiTexture } from 'pixi.js'

const nativeEvents = ['complete', 'start', 'end', 'dispose', 'interrupted']

const spineManager = spineData => {
  if (!spineData || !PixiSpine) return
  const obj = {}
  const animations = {}
  let spineSkeleton
  let animationState
  ...
  const setDefaultSkin = () => {
    ...
    if (!spineData.version) return
    spineSkeleton.setSkinByName(defaultSkin ? defaultSkin.name : Array.isArray(skins) ? skins[0].name : skins.default)
  }
  ...
  const generateEmptyAtlas = data => {
    ...
    Object.keys(skins.default).forEach(attachment => {
      Object.keys(skins.default[attachment]).forEach(texture => {
        allTextures[texture] = PixiTexture.EMPTY
      })
    })
    atlas.addTextureHash(allTextures)
    return atlas
  }

  const getSpineData = (rawSkeletonData, spineAtlas) => {
    const spineAtlasLoader = new PixiSpine.core.AtlasAttachmentLoader(spineAtlas)
    const spineJsonParser = new PixiSpine.core.SkeletonJson(spineAtlasLoader)
